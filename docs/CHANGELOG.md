...
### 🛠️ 2026-02-27 P0 性能优化实施
- **主进程 (Main Process)**: 引入日志缓冲区 (`BUFFER_FLUSH_INTERVAL = 100ms`)，有效合并高频 IPC 消息，降低通信开销。
- **渲染进程 (Renderer)**: 
  - 引入 `requestAnimationFrame` (rAF) 节流机制，确保 UI 更新频率与屏幕刷新率同步。
  - 集成 `react-window` 虚拟滚动，解决长日志渲染导致的 DOM 性能瓶颈。
  - 实现日志控制台自动滚动到底部逻辑。
